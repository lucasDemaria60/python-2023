...
def calculo():
    display_state = display.get()
    try:
        math_expression =  compile(display_state, 'calculadora profe.py', 'eval')
...
#Botones 2
Button(root, text="🚽",borderwidth=5, fg='red' , font=('arial', 25, "bold" ),command=lambda:limpiar()).grid(row=2, column=0, sticky=W+E)
...
Button(root, text="🧻",borderwidth=5, font=('italic',25 ), command=lambda:undo()).grid(row=2, column=1, sticky=W+E)
